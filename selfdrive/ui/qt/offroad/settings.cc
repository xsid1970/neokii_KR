...
DevicePanel::DevicePanel(QWidget* parent) : QWidget(parent) {
  ...
  main_layout->addWidget(horizontal_line());
  QHBoxLayout *git_layout = new QHBoxLayout(); //새로운 버튼 추가를 위한 레이아웃 변수 git_layout
  git_layout->setSpacing(30);
  main_layout->addWidget(horizontal_line()); // 구분선 구간

  QPushButton *panda_flashingBtn = new QPushButton("판다플래싱");
  panda_flashingBtn->setStyleSheet("height: 120px;border-radius: 15px;background-color: #00BFFF;");
  git_layout->addWidget(panda_flashingBtn);
  QObject::connect(panda_flashingBtn, &QPushButton::released, [=]() {
    if (ConfirmationDialog::confirm("판다플래싱 후 재부팅 합니다", this)) {
      std::system("/data/openpilot/panda_flashing.sh ''");
    }
  });
  main_layout->addLayout(git_layout); // 기본 main_layout 에 추가한다..
...
QWidget * network_panel(QWidget * parent) {
  ...
  auto gitrestBtn = new ButtonControl("깃 리셋", "실행");
  layout->addWidget(gitrestBtn);
  QObject::connect(gitrestBtn, &ButtonControl::released, [=]() {
    if (ConfirmationDialog::confirm("이온에서 직접 수정된 부분을\n초기화 합니다.", w)) {
  ...
  auto gitpull_cancelBtn = new ButtonControl("깃풀 취소", "실행");
  layout->addWidget(gitpull_cancelBtn);
  QObject::connect(gitpull_cancelBtn, &ButtonControl::released, [=]() {
    if (ConfirmationDialog::confirm("깃풀을 취소합니다.\n적용 후 자동 재부팅", w)) {
...
QWidget * community_panel() {
  ...
  supported_cars->addItem("[ 차량을 선택하세요 ]");
